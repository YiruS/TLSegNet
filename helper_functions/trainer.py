...
def run_training(
	...
	for i_iter in range(args.total_iterations):
		...
		for sub_i in range(args.iter_size):
			...
			if i_iter >= args.semi_start:
				D_out_semi = model_D(pred_softmax_target)
				semi_ignore_mask = (D_out_semi > args.mask_T).data.cpu().numpy()
				semi_gt = pred_softmax_target.data.cpu().numpy().argmax(axis=1)
			...
			# 	mask_T = torch.mean(pred_softmax_target).item()
			# 	semi_ignore_mask = (pred_confidence_value < mask_T)
			...
			loss_adv_target = gan_loss(D_out, generated_label)
			...
			# with history
			pool_source_label = history_pool_true.query(generated_label)
			loss_D = gan_loss(D_out, pool_source_label)
			...
			# with history
			pool_target_label = history_pool_fake.query(generated_label)
			loss_D = gan_loss(D_out, pool_target_label)
			loss_D = loss_D / args.iter_size / 2
			loss_D.backward()
